...
function SectionHeader({ tag, title, sub }: { tag: string; title: string; sub: string }) {
  return (
    <div className="mb-8">
      <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-muted-foreground">{tag}</div>
          ...
          {referrals.map((r) => (
            <div key={r.co} className="glass group rounded-2xl p-5 transition hover:-translate-y-1 hover:border-[var(--glow)]/40">
              <div className="flex items-start justify-between">
                <div className="grid h-10 w-10 place-items-center rounded-lg bg-white/5"><Building2 className="h-5 w-5 text-[var(--glow-2)]" /></div>
                <button className="flex items-center gap-1 rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-xs hover:border-[var(--glow)]/50">
          ...
          {rejections.map((r, i) => (
            <div key={i} className="glass relative rounded-2xl p-6">
              ...
              <div className="mt-5 flex items-center justify-between border-t border-white/5 pt-4 text-xs text-muted-foreground">
                ...
                <span className="rounded-full bg-white/5 px-2.5 py-1">{r.react}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="glass rounded-2xl p-6">
          ...
          <div className="mt-6 space-y-4">
            {[
              ...
              <div key={c.u} className="flex gap-3 rounded-xl border border-white/5 bg-white/[0.02] p-4">
